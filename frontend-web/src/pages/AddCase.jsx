...
import {
  ArrowLeft, User, MapPin, Phone, Syringe, Calendar, Clock,
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <form id="case-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <SectionHeader icon={User} title="Patient Details" subtitle="Section 1.2 — Personal information of the patient" color="bg-blue-600" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <InputField label="Patient Full Name" required>
                  <div className="relative">
                    <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input type="text" placeholder="e.g. Juan Dela Cruz" value={form.fullName}
              ...
              <InputField label="Age" required>
                <input type="number" placeholder="e.g. 25" min="0" max="120" value={form.age}
                  onChange={e => set('age', e.target.value)} required className={inputClass} />
              </InputField>
              <InputField label="Sex" required>
                <div className="relative">
                  <select value={form.sex} onChange={e => set('sex', e.target.value)} required className={selectClass}>
              ...
              <div className="md:col-span-2">
                <InputField label="Address" required>
                  <div className="relative">
                    ...
                    <input type="text" placeholder="e.g. Brgy. San Jose, Cebu City" value={form.address}
                      onChange={e => set('address', e.target.value)} required className={`${inputClass} pl-10`} />
                  </div>
                </InputField>
              </div>
              <InputField label="Contact Number" required>
                <div className="relative">
                  ...
                  <input type="text" placeholder="e.g. 09XX-XXX-XXXX" value={form.contact}
                    onChange={e => set('contact', e.target.value)} required className={`${inputClass} pl-10`} />
                </div>
              </InputField>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <SectionHeader icon={AlertCircle} title="Exposure Information" subtitle="Section 1.1 — Type of rabies exposure" color="bg-red-500" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField label="Type of Exposure" required>
                <div className="relative">
                  <select value={form.exposureType} onChange={e => set('exposureType', e.target.value)} required className={selectClass}>
              ...
              {/* Exposure Type Visual Indicators */}
              <div className="flex items-center gap-3">
          ...
          {/* ── C. Incident Logging ── */}
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <SectionHeader icon={FileText} title="Incident Logging" subtitle="Section 1.3 — Details of the incident" color="bg-amber-500" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField label="Date of Exposure" required>
                <div className="relative">
                  <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input type="date" value={form.dateOfExposure}
                    onChange={e => set('dateOfExposure', e.target.value)} required className={`${inputClass} pl-10`} />
                </div>
              </InputField>
              <InputField label="Time of Exposure" required>
                <div className="relative">
                  <Clock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input type="time" value={form.timeOfExposure}
                    onChange={e => set('timeOfExposure', e.target.value)} required className={`${inputClass} pl-10`} />
                </div>
              </InputField>
              <div className="md:col-span-2">
                <InputField label="Location of Incident" required>
                  <div className="relative">
                    <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input type="text" placeholder="e.g. Brgy. Lahug, near the market" value={form.location}
                      onChange={e => set('location', e.target.value)} required className={`${inputClass} pl-10`} />
                  </div>
                </InputField>
              </div>
              <InputField label="Animal Involved" required>
                <div className="relative">
                  ...
                  <select value={form.animalInvolved} onChange={e => set('animalInvolved', e.target.value)} required
              ...
              <InputField label="Animal Status" required>
                <div className="relative">
                  <select value={form.animalStatus} onChange={e => set('animalStatus', e.target.value)} required className={selectClass}>
          ...
          {/* ── D. Document Upload ── */}
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <SectionHeader icon={Upload} title="Document Upload" subtitle="Section 1.4 — Supporting documents (optional)" color="bg-emerald-500" />
            <label className={`flex flex-col items-center justify-center w-full h-36 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-200
              ${fileName ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 hover:border-blue-400 hover:bg-blue-50/40 bg-slate-50'}`}>
