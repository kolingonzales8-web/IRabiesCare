...
import { getAllCases, deleteCase as deleteCaseAPI, getCaseStats } from '../api/cases';
...
const StatusBadge = ({ status }) => {
  ...
  const dots = {
    ...
    Pending: 'bg-amber-500', Urgent: 'bg-red-500',
  };
  return (
...
const ExposureBadge = ({ type }) => {
  const map = {
    Bite: 'bg-red-50 text-red-700 border-red-200',
    Scratch: 'bg-orange-50 text-orange-700 border-orange-200',
    'Lick on Broken Skin': 'bg-yellow-50 text-yellow-700 border-yellow-200',
  };
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${map[type] || 'bg-slate-100 text-slate-600 border-slate-200'}`}>
      {type}
    </span>
  );
};
...
export default function Cases() {
  ...
  const handleRefresh = () => { fetchCases(); fetchStats(); };

  const handleDelete = async () => {
    setDeleting(true);
    try {
      await deleteCaseAPI(deleteId);
      setDeleteId(null);
      fetchCases();
      fetchStats();
    } catch (err) {
    ...
    <div className="h-screen flex flex-col bg-slate-50 overflow-hidden">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
            ...
            <h3 className="text-base font-bold text-slate-800 text-center mb-2">Delete Case?</h3>
            <p className="text-sm text-slate-500 text-center mb-6">This action cannot be undone. Case will be permanently removed.</p>
      ...
      <header className="shrink-0 bg-white border-b border-slate-200 px-6 h-[70px] flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">
            All registered rabies exposure cases
            {lastUpdated && <span className="ml-2 text-slate-300">· Updated {lastUpdated.toLocaleTimeString()}</span>}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg text-sm font-medium transition-colors">
      ...
      <div className="flex-1 flex flex-col overflow-hidden px-6 py-5">
        ...
        <div className="shrink-0 grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
          {[
            { label: 'Total Cases',  value: stats.total,     bg: 'bg-blue-50',    text: 'text-blue-700' },
        ...
        {/* Table Card — takes remaining height */}
        <div className="flex-1 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-0">
          ...
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-4 p-5 border-b border-slate-100">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input type="text" placeholder="Search by name or case ID..."
            ...
            <table className="w-full border-collapse">
              <thead className="sticky top-0 z-10">
                <tr className="border-b border-slate-100 bg-slate-50">
                  {['Case ID', 'Patient Name', 'Age', 'Sex', 'Contact', 'Exposure Type', 'Body Part', 'Animal', 'Date Exposed', 'Status', 'Actions'].map(h => (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={11} className="px-4 py-16 text-center text-red-400">
                      ...
                      <button onClick={fetchCases} className="mt-2 text-xs text-blue-500 underline">Retry</button>
                    </td>
                  </tr>
                ) : cases.length === 0 ? (
                  <tr>
                  ...
                  <tr key={c._id}
                    ...
                    <td className="px-4 py-3.5">
                      <span className="font-bold text-blue-600 text-xs bg-blue-50 px-2 py-1 rounded-lg">#{c.caseId}</span>
                    </td>
                    <td className="px-4 py-3.5">
                      <p className="font-semibold text-slate-800 text-sm whitespace-nowrap">{c.fullName}</p>
                    </td>
                    <td className="px-4 py-3.5 text-slate-600 text-sm">{c.age}</td>
                    <td className="px-4 py-3.5 text-slate-600 text-sm">{c.sex}</td>
                    <td className="px-4 py-3.5 text-slate-500 text-xs">{c.contact || '—'}</td>
                    <td className="px-4 py-3.5"><ExposureBadge type={c.exposureType} /></td>
                    <td className="px-4 py-3.5 text-slate-600 text-sm">{c.bodyPartAffected || '—'}</td>
                    <td className="px-4 py-3.5 text-slate-600 text-sm">{c.animalInvolved}</td>
                    <td className="px-4 py-3.5 text-slate-500 text-xs whitespace-nowrap">{formatDate(c.dateOfExposure)}</td>
                    <td className="px-4 py-3.5"><StatusBadge status={c.status} /></td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => navigate(`/cases/view/${c._id}`)}
                        ...
                        <button onClick={() => navigate(`/cases/edit/${c._id}`)}
          ...
          {totalPages > 1 && (
            <div className="shrink-0 flex items-center justify-between px-5 py-4 border-t border-slate-100 bg-white">
              <p className="text-xs text-slate-500">
                Showing <span className="font-semibold text-slate-700">{(page - 1) * ITEMS_PER_PAGE + 1}–{Math.min(page * ITEMS_PER_PAGE, total)}</span> of <span className="font-semibold text-slate-700">{total}</span> cases
              </p>
